...
public class PatternMarkerProcessor implements IMarkerProcessor {
	...
	/**
	 * Creates a PatternMarker object, using given specifications from the application.
	 * 
	 * @param uid A unique identifier string, which can be used to represent a marker.
	 * @param segments The number of segments dictating the resolution of the pattern.
	 * 					This number is set when the pattern is created.
	 * @param path	The path to the file containing pattern data.
	 * @return PatternMarker Returns a PatternMarker object to the application, returns null if a problem exists
	 */
	public PatternMarker createMarkerObject(String uid, int segments, String path) {
		...
		else return new PatternMarker(uid, code);
		
	}
	
	/**
	 * Gets the marker at a specified Array Position. Due to the fact that NyARToolKit references 
	 * detected markers based on their position in a primitive Array, we need to be able to relate
	 * a marker object to this array position.
	 * 
	 * @param positionInArray The primitive array position for which we want the marker.
	...
	/**
	 * Creates the List of NyARCodes which are used in the NyARToolKit detection program. This
	 * Code list forms the basis for all marker detection, through the ordering of the array.
	 * 
	...
	public NyARCode[] createARCodesList(){
		...
		for (int i=0; i<markerList.size(); i++){
			...
			//set the position of this marker in the codes array (to allow us to detect what marker we are using later)
